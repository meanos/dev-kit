//snw.Window.get().evalNWBin(null, '/usr/meanOs/iXAdjust/apps/mean.devkit.app/editor.bin');
...
var selectedProjectType = 'meanOsApp'; //What type of project to create when user creates a new project
var allDirectories = [];
...
function newmeanOsApp(projectDirectory) {
	...
	closePopupDialogue(true);
	    ncp('/usr/meanOs/systemX/Shared/CoreAPP/mean.template.app', projectDirectory+"/", function (err) {
  		if (err) {
    			console.error(err);
  		} else {
   			 console.log("success!");
			...
			pjson.id = "mean."+$("#newProjectName").val()+"."+pjson.author+".app";
...
function createNewProject() {
	...
	if (selectedProjectType == "meanOsApp")
		newmeanOsApp(selectedDirectoryForNewProject+"/"+$("#newProjectName").val());
}

function selectProjectType(projectType,pureName) {
	if ((projectType == "meanOsApp") || (projectType == "emptyProject")) { //Project from malicious/incorrect input
...
$("#accordionContainerSettings").prepend('<div class="panel-group block hidden" id="accordionSettings'+projectNo+'">'+
				...
				'<h4 class="block-title">Compiling meanOs Apps</h4>'+
				'<div id="projectCompilingWarning" class="alert alert-info" style="text-align: left;">'+
                        		        'Please note, compiling to binary might require the project to be recompiled for every major release of meanOs.'+
                                    '</div>'+
...
console.log("gets hereK");
	if (directory.indexOf("/usr/meanOs/systemX/apps/") == 0)
...
var dirs = '/usr/meanOs/systemX/apps/mean.welcome.app';
...
function newEmptyEditor() {
	$("#projectFiles0 > ul").prepend("<li><span class='fileIcon'><img class='svgIcon' src='../mean.files.app/icons/blank.png'></span><a href='#' icon='../mean.files.app/icons/blank.png' class='projectFile' onclick='openFile("+allFilesUrls.length+",&quot;../mean.files.app/icons/blank.png&quot;,this)'>Untitled</a></li>");
...
//setTimeout(function(){  }, 1000);
	//newEditor("","","../mean.files.app/icons/blank.png",true,$("#projectFiles0 > ul").first(),true);
...
function assignIcon(fileName,isDirectory) {
	...
	if (isDirectory)
		var icon = "../mean.files.app/icons/"+win.resolveFileType("*folder*",true);
	else
		var icon = "../mean.files.app/icons/"+win.resolveFileType(fileName.split('.').pop(),true);
...
var getAllAvailableLanguages = function() {
	var path = "/usr/meanOs/systemX/apps/mean.devkit.app/node_modules/monaco-editor/dev/vs/basic-languages";
...
function generateBinary(currentFilePosition,buildPackage) {
	...
	var exportToBin = execSync('/usr/meanOs/NodeJs/nwjc "'+filesToBuild[currentFilePosition]+'" "'+currentFileAsBin+'"');
	currentFilePosition++;
...
require.config({ paths: { 'vs': 'node_modules/monaco-editor/min/vs' }});
	...
	/*monaco.editor.defineTheme('meanOs-Dark', {
		...
		monaco.editor.setTheme('meanOs-Dark');*/
